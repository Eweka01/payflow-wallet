...
print_summary() {
  ...
  echo ""
  echo "  2. Paste WAF + ACM ARNs into helm/payflow/values-dev.yaml → commit → push"
  echo "     (ArgoCD will sync and the ALB will gain WAF protection)"
  echo ""
  echo "  3. Run setup-cluster.sh from your LOCAL machine:"
  echo ""
  echo "       ./infra-scripts/setup-cluster.sh"
  echo ""
  echo "     Installs: ArgoCD (internal NLB) · ESO · metrics-server · Image Updater"
  ...
  echo ""
  echo "  4. Trigger the first CI run to push images to ECR:"
  echo ""
  echo "       git commit --allow-empty -m \"chore: trigger initial CI run\""
  echo "       git push origin main"
  echo "       gh run view --watch"
  echo ""
  echo "     Image Updater detects new tags within 2 minutes → ArgoCD rolling update"
  echo ""
